...
class TestWizardPlaceholderStep:
    """Integration tests for placeholder steps (9+)."""
...
class TestWizardStateExtended:
    """Tests for extended WizardState fields."""

    def test_default_skip_storage(self) -> None:
        state = WizardState()
        assert state.skip_storage is False

    def test_default_storage_path(self) -> None:
        state = WizardState()
        assert state.storage_path == ""

    def test_default_admin_pin(self) -> None:
        state = WizardState()
        assert state.admin_pin == ""

    def test_default_user_pin(self) -> None:
        state = WizardState()
        assert state.user_pin == ""

    def test_set_storage_path(self) -> None:
        state = WizardState()
        state.storage_path = "/Volumes/backup"
        assert state.storage_path == "/Volumes/backup"

    def test_set_pins(self) -> None:
        state = WizardState()
        state.admin_pin = "12345678"
        state.user_pin = "123456"
        assert state.admin_pin == "12345678"
        assert state.user_pin == "123456"


class TestWizardStorageStepIntegration:
    """Integration tests for the storage setup step (step 5)."""

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_storage_step_renders(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            skip_cb = screen.query_one("#check-skip-storage", Checkbox)
            path_input = screen.query_one("#input-storage-path", Input)
            assert skip_cb is not None
            assert path_input is not None

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_storage_skip_enables_next(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_storage_path_enables_next(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            # Enter a path
            path_input = screen.query_one("#input-storage-path", Input)
            path_input.value = "/Volumes/backup"
            await pilot.pause()
            await pilot.pause()
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_storage_captures_state(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            screen.query_one("#input-storage-path", Input).value = "/tmp/backup"
            await pilot.pause()
            screen._capture_step_state()

            assert screen._state.storage_path == "/tmp/backup"


class TestWizardGenerationStepIntegration:
    """Integration tests for the key generation step (step 6)."""

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_generation_no_controller_shows_preview_message(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            assert "preview" in text.lower() or "No controller" in text

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_generation_with_controller_shows_button(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            gen_btn = screen.query_one("#btn-generate", Button)
            assert gen_btn is not None
            assert screen._step_complete is False

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_generation_already_done_shows_key_id(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            screen = WizardScreen(controller=mock_controller)
            screen._state.key_id = "ABC123DEF456"
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_generation_shows_config_summary(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            screen = WizardScreen(controller=mock_controller)
            screen._state.identity = "Alice <alice@test.com>"
            screen._state.key_type = KeyType.ED25519
            screen._state.expiry_years = 3
            screen._state.passphrase = SecureString("testpassphrase!!")
            await pilot.app.push_screen(screen)
            await pilot.pause()
            await pilot.pause()
            ...
            assert "ed25519" in text.lower() or "ED25519" in text

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_generation_execute_success(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        from yubikey_init.tui.app import YubiKeyManagerApp
        from yubikey_init.types import KeyInfo, KeyUsage, Result, SubkeyInfo
        ...
        mock_controller.generate_master_key.return_value = Result.ok(key_info)
        mock_controller.generate_all_subkeys.return_value = Result.ok(subkeys)
        ...
        async with app.run_test() as pilot:
            ...
            assert screen._state.key_id == "DEADBEEF1234"
            assert screen._state.subkey_count == 3
            assert screen._step_complete is True

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_generation_execute_master_key_fails(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        mock_verify.return_value = all_passed_report
        mock_controller.generate_master_key.return_value = Result.err(RuntimeError("GPG not found"))
        ...
        async with app.run_test() as pilot:
            ...
            assert screen._state.key_id is None
            assert screen._step_complete is False

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_generation_execute_subkey_fails(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        mock_controller.generate_master_key.return_value = Result.ok(key_info)
        mock_controller.generate_all_subkeys.return_value = Result.err(
            RuntimeError("Subkey failed")
        )
        ...
        async with app.run_test() as pilot:
            ...
            # Master key should still be set
            assert screen._state.key_id == "MASTER123"
            # But subkey_count stays at 0
            assert screen._state.subkey_count == 0
            # Should still allow advancing
            assert screen._step_complete is True


class TestWizardBackupStepIntegration:
    """Integration tests for the backup creation step (step 7)."""

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_backup_step_renders(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            screen._step_complete = True
            screen._current_step = 7
            screen._run_step()
            await pilot.pause()
            await pilot.pause()
            await pilot.pause()

            assert screen._current_step == 7
            assert screen._step_complete is True

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_backup_shows_skip_message(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            screen = WizardScreen(controller=mock_controller)
            screen._state.skip_storage = True
            await pilot.app.push_screen(screen)
            await pilot.pause()
            await pilot.pause()
            ...
            content = screen.query_one("#step-content", Vertical)
            text = " ".join(str(child.render()) for child in content.children)
            assert "skipped" in text.lower() or "Skipped" in text

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_backup_shows_path(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            screen = WizardScreen(controller=mock_controller)
            screen._state.skip_storage = False
            screen._state.storage_path = "/Volumes/backup"
            await pilot.app.push_screen(screen)
            await pilot.pause()
            await pilot.pause()
            ...
            content = screen.query_one("#step-content", Vertical)
            text = " ".join(str(child.render()) for child in content.children)
            assert "/Volumes/backup" in text

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_backup_with_key_shows_button(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            screen._state.storage_path = "/tmp/backup"
            screen._state.passphrase = SecureString("testpassphrase!!")
            await pilot.app.push_screen(screen)
            await pilot.pause()
            await pilot.pause()
            ...
            backup_btn = screen.query_one("#btn-backup", Button)
            assert backup_btn is not None

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_backup_already_done(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            content = screen.query_one("#step-content", Vertical)
            text = " ".join(str(child.render()) for child in content.children)
            assert "already" in text.lower() or "complete" in text.lower()

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_backup_execute_success(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        manifest = BackupManifest(
            ...
            fingerprint="ABCD" * 10,
            identity="Test <test@test.com>",
            files=["key.gpg", "subkeys.gpg", "revoke.asc"],
            backup_path=Path("/tmp/backup"),
        )
        mock_controller.create_backup.return_value = Result.ok(manifest)
        ...
        async with app.run_test() as pilot:
            ...
            await pilot.click("#btn-backup")
            await pilot.pause()
            await pilot.pause()
            await pilot.pause()

            assert screen._state.backup_complete is True
            assert screen._step_complete is True

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_backup_execute_failure(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        mock_verify.return_value = all_passed_report
        mock_controller.create_backup.return_value = Result.err(RuntimeError("Disk full"))
        ...
        async with app.run_test() as pilot:
            ...
            assert screen._state.backup_complete is False
            assert screen._step_complete is False

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_backup_no_path_skips(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            # No backup path and skip_storage=False: should show skip message
            assert screen._step_complete is True
            content = screen.query_one("#step-content", Vertical)
            text = " ".join(str(child.render()) for child in content.children)
            assert "No backup path" in text or "Skipping" in text


class TestWizardTransferStepIntegration:
    """Integration tests for the YubiKey transfer step (step 8)."""

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_transfer_step_renders(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            assert screen._current_step == 8
            admin_input = screen.query_one("#input-admin-pin", Input)
            user_input = screen.query_one("#input-user-pin", Input)
            assert admin_input is not None
            assert user_input is not None

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_transfer_valid_pins_enable_next(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            screen.query_one("#input-admin-pin", Input).value = "12345678"
            screen.query_one("#input-user-pin", Input).value = "123456"
            await pilot.pause()
            await pilot.pause()
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_transfer_short_pins_block_next(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            screen.query_one("#input-admin-pin", Input).value = "123"
            screen.query_one("#input-user-pin", Input).value = "12"
            await pilot.pause()
            await pilot.pause()
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_transfer_captures_state(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            screen.query_one("#input-admin-pin", Input).value = "87654321"
            screen.query_one("#input-user-pin", Input).value = "654321"
            await pilot.pause()

            screen._capture_step_state()
            assert screen._state.admin_pin == "87654321"
            assert screen._state.user_pin == "654321"


class TestWizardVerificationStepIntegration:
    """Integration tests for the verification step (step 9)."""

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_verification_step_renders(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            assert screen._current_step == 9
            assert screen._step_complete is True

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_verification_shows_identity(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_verification_shows_missing_identity(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_verification_shows_storage_skipped(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_verification_shows_pins_configured(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
...
class TestWizardSummaryStepIntegration:
    """Integration tests for the summary step (step 10)."""

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_summary_step_renders(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            assert screen._current_step == 10
            assert screen._step_complete is True

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_summary_next_disabled(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            ...
            # Next should be disabled on the last step
            next_btn = screen.query_one("#btn-next", Button)
            assert next_btn.disabled is True

    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_summary_shows_full_config(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            screen = WizardScreen(controller=mock_controller)
            screen._state.identity = "Bob <bob@test.com>"
            screen._state.key_type = KeyType.RSA4096
            screen._state.expiry_years = 5
            screen._state.storage_path = "/Volumes/backup"
            screen._state.admin_pin = "12345678"
            screen._state.user_pin = "123456"
            await pilot.app.push_screen(screen)
            ...
            text = " ".join(str(child.render()) for child in content.children)
            assert "Bob" in text
            assert "rsa4096" in text.lower() or "RSA4096" in text
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_summary_shows_skipped_storage(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
    ...
    @pytest.mark.asyncio
    @patch("yubikey_init.tui.screens.wizard.verify_environment")
    async def test_summary_shows_default_storage(
        self,
        mock_verify: MagicMock,
        all_passed_report: EnvironmentReport,
        mock_controller: MagicMock,
        ...
        async with app.run_test() as pilot:
            screen = WizardScreen(controller=mock_controller)
            # skip_storage=False, storage_path=""
            await pilot.app.push_screen(screen)
            await pilot.pause()
            await pilot.pause()
...
class TestWizardStateExecutionFields:
    """Tests for WizardState execution result fields."""

    def test_default_key_id(self) -> None:
        state = WizardState()
        assert state.key_id is None

    def test_default_subkey_count(self) -> None:
        state = WizardState()
        assert state.subkey_count == 0

    def test_default_backup_complete(self) -> None:
        state = WizardState()
        assert state.backup_complete is False

    def test_set_key_id(self) -> None:
        state = WizardState()
        state.key_id = "ABC123"
        assert state.key_id == "ABC123"
